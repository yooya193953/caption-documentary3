#!/usr/bin/env python3
"""
Comprehensive JSONL Session File Extractor v2
...
class FileStateTracker:
    ...
    def add_read(self, filepath: str, content: str, timestamp: str, is_partial: bool = False):
        ...
        if is_partial:
            ...
            print(f"  [PARTIAL READ] {filepath} ({len(content)} bytes)")
        else:
            self.files[filepath].append({
                'timestamp': timestamp,
                'content': content,
...
def extract_from_tool_result(result_data, timestamp: str, tracker: FileStateTracker):
    ...
    # Type: 'edit' - Edit operation result
        ...
        if filepath and original_file:
            ...
            # We should track this as a read of the pre-edit state
            tracker.add_read(filepath, original_file, timestamp, False)


def parse_jsonl_line(line: str, tracker: FileStateTracker, line_num: int):
...
def main():
    ...
    print("=" * 80)
    print("COMPREHENSIVE JSONL SESSION FILE EXTRACTOR v2")
    print("=" * 80)
    ...
    with open(session_file, 'r', encoding='utf-8') as f:
        for i, line in enumerate(f, 1):
            print(f"\nLine {i}/{line_count}:")
            parse_jsonl_line(line.strip(), tracker, i)
